...
import {
  existsSync, readFileSync, writeFileSync, copyFileSync, statSync,
} from 'node:fs';
...
const die = (s) => { console.error(`\n\x1b[31m✗ ${s}\x1b[0m`); process.exit(1); };

function git(args, cwd = SITE, allowFail = false) {
  const r = spawnSync('git', ['-C', cwd, ...args], { encoding: 'utf8', maxBuffer: 1 << 24 });
...
// ---------------- 0. 体检 ----------------
step(0, '体检');
for (const [dir, what] of [[CV, '简历源码仓库'], [SITE, '站点仓库']]) {
  need(existsSync(join(dir, '.git')), `${what}不存在或不是 git 仓库：${dir}（可用 CV_DIR=... 覆盖路径）`);
}
for (const j of JOBS) {
...
ok(`站点仓库 ${SITE}`);
if (opts.push) {
  const auth = spawnSync('ssh', ['-T', '-o', 'StrictHostKeyChecking=accept-new', '-o', 'BatchMode=yes',
    '-o', 'ConnectTimeout=10', `git@github.com`], { encoding: 'utf8' });
  const msg = (auth.stdout || '') + (auth.stderr || '');
  if (/successfully authenticated/.test(msg)) ok('GitHub SSH 可用');
  else warn(`GitHub SSH 未通过（push 可能失败）：${msg.split('\n')[0] || 'no response'}`);
}
...
if (!opts.compile) {
  ...
  const runLatexmk = (j, force) => spawnSync('latexmk',
    [...(force ? ['-g'] : []), '-xelatex', '-interaction=nonstopmode', '-file-line-error', j.tex],
  ...
  for (const j of JOBS) {
    ...
    if (r.status !== 0) {
      ...
      const errs = logTxt.split('\n').filter((l) => /^!|Error:/.test(l)).slice(0, 6).join('\n    ');
      die(`${j.label} 编译失败：\n    ${errs || '（见 .log）'}`);
    }
    need(existsSync(join(CV, j.pdf)), `${j.label} 编译完找不到 ${j.pdf}`);
    // latexmk 认为“无依赖变化”时不会重写 pdf；再强制跑一次，仍不新才报错
    if (statSync(join(CV, j.pdf)).mtimeMs < statSync(join(CV, j.tex)).mtimeMs - 1000) {
      const r2 = runLatexmk(j, true);
      need(r2.status === 0 && statSync(join(CV, j.pdf)).mtimeMs >= statSync(join(CV, j.tex)).mtimeMs - 1000,
    ...
    if (pages && Number(pages[1]) > 1) warn(`${j.label} 编译成功，但现在是 ${pages[1]} 页（原本要求一页）`);
    else ok(`${j.label} → ${j.pdf}`);
  }
}
for (const j of JOBS) {
  need(existsSync(join(CV, j.pdf)), `缺成品 ${join(CV, j.pdf)}（先编译，或去掉 --no-compile）`);
...
const cvPaths = JOBS.flatMap((j) => [j.tex, j.pdf]);
{
  ...
  const others = git(['status', '--porcelain'], CV).split('\n').filter(Boolean).length
    - dirty.split('\n').filter(Boolean).length;
  if (!dirty) {
    ok('两个 .tex/.pdf 相对上次提交没有变化，无需提交');
  } else if (DRY || !opts.cvCommit) {
    warn(`有改动未提交（${DRY ? 'dry-run' : '--no-cv-commit'}）：\n    ${dirty.replace(/\n/g, '\n    ')}`);
  } else {
    const msg = opts.msg || `发布简历：更新中英文通用简历 ${new Date().toISOString().slice(0, 10)}`;
    git(['add', '--', ...cvPaths], CV);          // pdf 首次纳入跟踪时也不会漏
    git(['commit', '-m', msg, '--', ...cvPaths], CV);
    ok(`已提交 ${git(['rev-parse', '--short', 'HEAD'], CV)} — ${msg}`);
    if (others > 0) warn(`CV 仓库还有 ${others} 处其他改动没提交（本次有意不碰，见 git工作流.md）`);
  }
}
...
for (const j of JOBS) {
  ...
  else { copyFileSync(join(CV, j.pdf), dst); ok(`${j.site} ← ${j.pdf}`); }
}
...
for (const f of [CAREER.content, CAREER.template]) {
  ...
  if (opts.bust) {
    for (const j of JOBS) {
      ...
      const re = new RegExp(`assets/resumes/${j.site.split('/').pop().replace('.', '\\.')}(?:\\?v=[0-9a-f]+)?`, 'g');
      after = after.replace(re, `assets/resumes/${j.site.split('/').pop()}?v=${tag}`);
    }
  }
  if (after !== before) {
    if (DRY) { warn(`dry-run：将改写 ${f} 的链接版本号`); continue; }
...
{
  const tpl = existsSync(join(SITE, CAREER.template)) ? readFileSync(join(SITE, CAREER.template), 'utf8') : '';
  const cont = existsSync(join(SITE, CAREER.content)) ? readFileSync(join(SITE, CAREER.content), 'utf8') : '';
  for (const j of JOBS) {
    const name = j.site.split('/').pop();
    const linked = cont.includes(name) || tpl.includes(name);
    if (linked) ok(`${name} 有下载入口`); else warn(`线上页面目前**没有指向 ${name} 的入口**——文件传了也下不到。恢复链接：git checkout HEAD -- ${CAREER.content} 后重跑，或在编辑台里加回按钮`);
  }
}
if (touchedContent && !DRY) {
  const r = spawnSync('node', [join(SITE, 'tools', 'build.mjs'), 'build'],
    { cwd: SITE, encoding: 'utf8', env: { ...process.env, PI_SITE: 'career' } });
  if (r.status !== 0) die(`重建 career 页面失败：${r.stdout || ''}${r.stderr || ''}`);
...
const sitePaths = [
  ...JOBS.map((j) => j.site),
  CAREER.content, CAREER.index, CAREER.zh, CAREER.template,
].filter((f, i, a) => existsSync(join(SITE, f)) && a.indexOf(f) === i);
{
  ...
  if (dirty) {
    ...
    // ① 只看简历相关的文件
    const pdfPaths = JOBS.map((j) => j.site).filter((f) => existsSync(join(SITE, f)));
    const textPaths = [CAREER.content, CAREER.index, CAREER.zh, CAREER.template]
      .filter((f) => dirty.split('\n').some((l) => l.includes(f)));

    // ② 这些生成物里除了简历链接，还有多少行其他文案要一起上线
    const other = textPaths.length
      ? git(['diff', '--', ...textPaths]).split('\n')
        .filter((l) => /^[-+][^-+]/.test(l) && !/Resume_(EN|CN)\.pdf/i.test(l))
        .filter((l) => !/^[-+]\s*[{}\[\],"']*(en|zh)"?:\s*"?<a href="assets\/resumes/.test(l))
      : [];
    ...
    if (other.length && textPaths.length) {
      ...
      for (const l of other.slice(0, 6)) console.log(`    \x1b[90m${l.slice(0, 130)}\x1b[0m`);
      if (other.length > 6) console.log(`    \x1b[90m… 另有 ${other.length - 6} 行\x1b[0m`);
      let a = null;
      if (opts.yes) a = 'a';
      else if (opts.pdfOnly) a = 'p';
      else {
        a = await ask('  [a] 一起上线（简历+这些文案） / [p] 只上传两个 PDF / 其他文字=取消 ： ');
      }
      if (a === null) {
        die('非交互环境不能代你决定：重跑时加 --yes（文案跟传）或 --pdf-only（只传 PDF）');
      }
      if (a === 'cancel' || a === 'c' || (a !== 'a' && a !== 'p' && a !== '')) {
        console.log('\n  已取消：未提交、未推送。本地改动都在。');
        process.exit(0);
      }
      includeText = a !== 'p';
    ...
    const commitPaths = includeText ? sitePaths.filter((f) => existsSync(join(SITE, f))) : pdfPaths;
    const msg = opts.msg || `发布中英文简历 ${new Date().toISOString().slice(0, 10)}${includeText ? '（含待发布页面文案）' : ''}`;
    git(['add', '--', ...commitPaths]);
    git(['commit', '-m', msg, '--', ...commitPaths]);
    ok(`已提交 ${git(['rev-parse', '--short', 'HEAD'])} — ${msg}`);
    if (!includeText) warn('只传了 PDF：?v= 破缓存没提交，线上旧链接会命中缓存。想破缓存下次选 [a]');
    const left = git(['status', '--porcelain']).split('\n').filter(Boolean).length;
    if (left > 0) warn(`站点还有 ${left} 处未提交改动，本次**没有**一起发布（编辑台的 🚀 会全量 add，这里刻意只传简历）`);
    }
  } else {
...
if (!opts.verify || DRY) {
  ...
  const want = Object.fromEntries(JOBS.map((j) => [j.site, md5(join(CV, j.pdf))]));
  const deadline = Date.now() + 300_000;
  const pending = new Set(Object.keys(want));
  while (pending.size && Date.now() < deadline) {
    for (const sitePath of [...pending]) {
      const url = `${PAGES}/${sitePath}?t=${Date.now()}`;
      const r = spawnSync('curl', ['-sL', '--max-time', '25', '-o', '/tmp/_remote_resume.pdf', '-w', '%{http_code}', url],
        { encoding: 'utf8' });
      if (r.stdout === '200' && existsSync('/tmp/_remote_resume.pdf') && md5('/tmp/_remote_resume.pdf') === want[sitePath]) {
        ok(`线上已是最新：${sitePath}`);
        pending.delete(sitePath);
      }
    }
    if (pending.size) { process.stdout.write('  …等 GitHub Pages 部署（约 30–90 秒）\r'); await sleep(15_000); }
    if (Date.now() > deadline) break;
  }
  for (const sitePath of pending) {
    warn(`线上还没刷新：${PAGES}/${sitePath} —— 稍后手动打开确认，或去 https://github.com/WshAzad/WshAzad.github.io/actions 看 Pages 部署任务`);
  }
}
